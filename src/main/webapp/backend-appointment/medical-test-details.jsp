...
if (testRecordId != null) {
	...
	if (medicalTestRecord != null) {
		...
		Map<String, String> inputs = (Map<String, String>) session.getAttribute("inputFiledsData");
		test_result = SessionMapUtils.getFiledValue(inputs, "medical_test_id");
		session.removeAttribute("inputFiledsData");
	} else {
						...
						<div class="col-md-12 grid-margin stretch-card">
							<div class="card">
								<div class="card-body">
									...
									<div class="my-5 px-5">
										<h4 class="pl-5 mb-3">Manage medical test record details</h4>
										<form class="pt-3" action="update-test-record" method="post">
											<div class="row">
												<div class="col-12">
													<input type="hidden" name="test_record_id"
														value="<%=testRecordId%>">
													<div class="form-group">
														<label for="test_result" class="font-weight-500">Medical
															Test Result</label>
													...
													<div class="form-group">
														<label for="test_status" class="font-weight-500">Medical
															Test Status</label> <select
															class="form-select form-control form-control-lg"
															id="test_status" name="test_status"
															aria-label="Medical Test Status">
															<%
															for (Status statusData : statusList) {
																String selected = (status_id == statusData.getId()) ? "selected" : "";
															%>
															<option <%=selected%> value="<%=statusData.getId()%>"><%=statusData.getName()%></option>
															<%
															}
															%>
														</select>
